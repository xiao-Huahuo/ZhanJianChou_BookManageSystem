...
namespace AdminRoutes
{
    void register_routes(crow::SimpleApp &app, Database &db)
    {
        ...
        // POST 添加新书
        CROW_ROUTE(app, "/api/admin/books").methods("POST"_method, "OPTIONS"_method)([&db, build_response](const crow::request &req)
                                                                                     {
                if (req.method == "OPTIONS"_method) {
                    auto res = crow::response(204);
                    add_cors_headers(res);
        ...
        // 获取所有用户
        CROW_ROUTE(app, "/api/admin/users").methods("GET"_method)([&db](const crow::request &req)
                                                                  {
                if (!is_admin(req, db)) {
                ...
                return res; });
        // 更新用户信息
        CROW_ROUTE(app, "/api/admin/users/<string>").methods("PUT"_method)([&db](const crow::request &req, const std::string &username)
                                                                           {
                if (!is_admin(req, db)) {
                    return crow::response(403, "{\"success\": false, \"message\": \"Forbidden\"}");
                }
